...
import * as FakeTradeActions from 'actions/trade.js';
...
class Trade extends Component{
        ...
        <div className="fl-block gray-bg">
          ...
          <div className="flex-wrapper">
            ...
            <div className="item-80-100">
              ...
              <div className="market-k-chart item-inner" style={{borderTop: '1px solid #eee', marginTop: 0}}>
              ...
              <div className="flex-wrapper" style={{width: '100%'}}>
                ...
                <div className="item-30-100" style={{ paddingTop: '15px'}}>
                  <div className="coin-market item-inner" style={{height: 'auto', padding: '10px 0'}}>
                    <div className="table-wrapper">
                        <div style={{textAlign: 'center', height: '30px', borderBottom: '1px solid #eee'}}>
                          <span>{intl.get('currentprice')}　<span className="price-amount">
                            { currentPrice }
                            </span>
                          <span className="unit">{config.CURRENCY}</span></span>
                          <span> ≈ {intl.get('currentpriceNum', { amount: (currentPriceLocale) })}</span>
                        </div>
                        <Table basic="very" textAlign="center" className="no-border-table order-book-table" style={{borderLeft: '1px solid #eee',marginTop: 0}}>
                          <Table.Header>
                            <Table.Row>
                              <Table.HeaderCell></Table.HeaderCell>
  ...
  handleSocketData(blob){
    ...
    if(this.state.suscribe_success){
      ...
      if( blob.result.code === 0){
        ...
        Toast.error(blob.result.msg || '订阅失败！')
      }
    }
  ...
  handleOrder(blob){
    ...
    new_out_order_list = asks.slice(0, 5).reverse();
    new_in_order_list = bids.slice(0, 5);
    console.warn('订单数据一条');
    console.warn('买盘数据：' );
        ...
        dealSocket.onmessage = function(event){
            ...
            if(self.state.deal_suscribe_success){
              ...
              if( blob.result.code === 0){
                ...
                Toast.error(blob.result.msg || '成交队列订阅失败')
              }
            }
        };
        ...
        dailySocket.onmessage = function(event){
            ...
            if(self.state.daily_suscribe_success ){
              ...
              if( blob.result.code === 0){
                ...
                Toast.error(blob.result.msg || '行情订阅失败')
              }
            }
        };
        ...
        orderSocket.onmessage = function(event){
            ...
            if(self.state.order_suscribe_success){
              ...
              if( blob.result.code === 0){
                ...
                Toast.error(blob.result.msg || '买卖盘队列订阅失败')
              }
            }
        };
